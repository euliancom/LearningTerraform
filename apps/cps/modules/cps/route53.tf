...
resource "aws_route53_record" "www" {
  ...
  records = ["${element(module.cps_aws_elb.elb_dns_name,count.index)}"]
}
...
resource "aws_route53_record" "app" {
  ...
  ttl = "300"
  count   = 1
  records = ["${element(module.cps_aws_elb_apache.elb_dns_name,count.index)}"]
}
...
resource "aws_route53_record" "app_jscape" {
  ...
  ttl = "300"
  count   = 1
  records = ["${element(module.cps_aws_elb_jscape.elb_dns_name,count.index)}"]
}                                                                                                                                                                                       
